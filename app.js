...
function calcSFF() {
  ...
  if (!isNaN(s) && !isNaN(d) && (s + d) > 0) {
    const sff = ((s / (s + d)) * 100).toFixed(1);
    document.getElementById('sff').value = sff;
    showSFF();
  }
}
...
function calcRVSP() {
  ...
  if (isNaN(v) || isNaN(rap)) { hideResult('rvsp-result'); return; }
  const val = 4 * v * v + rap;
  stored.rvsp = val;
...
function calcPASP() {
  ...
  const ps = parseFloat(document.getElementById('ps-grad').value) || 0;
  ...
  showResult('pasp-result', {
    ...
    value: fmt(val),
    unit: 'mmHg',
    formula: ps > 0 ? `RVSP(${rvsp}) − PS gradient(${ps}) = ${fmt(val)}` : `= RVSP (no pulmonic stenosis)`
  });
}
...
function calcMPAP() {
  const pasp = parseFloat(document.getElementById('pasp-m1').value);
  const padp = parseFloat(document.getElementById('padp-m1').value);
  ...
  if (!isNaN(pasp) && !isNaN(padp)) {
    ...
    cards.push({ label: 'Method 1: ⅓PASP + ⅔PADP', val: v });
  }
  if (!isNaN(prPeak) && !isNaN(rap2)) {
    const v = 4 * prPeak * prPeak + rap2;
    if (first === null) first = v;
    cards.push({ label: 'Method 2: 4(PR peak)² + RAP', val: v });
  }
  if (!isNaN(act)) {
    const v = 79 - 0.45 * act;
    if (first === null) first = v;
